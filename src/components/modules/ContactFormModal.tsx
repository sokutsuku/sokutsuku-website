...
interface FormData {
  name: string;
  company?: string; // 任意項目なので ? を追加
  phone?: string;   // 任意項目なので ? を追加
  email: string;
  message: string;
}
...
const ContactFormModal: React.FC<ContactFormModalProps> = ({ isOpen, onClose }) => {
  const {
    ...
    handleSubmit,
    reset, // フォームリセット用
    formState: { errors, isSubmitting }, // isSubmitting は react-hook-form が提供
  } = useForm<FormData>({
    mode: 'onChange', // バリデーションをいつ行うか (入力変更時)
    defaultValues: { // 初期値
      name: '',
      company: '',
      phone: '',
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      reset(); // モーダルが開かれたらフォームをリセット
      setSubmitStatus(null); // 送信ステータスもリセット
    } else {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    // ここに実際のフォーム送信処理を実装します (例: APIへのPOSTリクエスト)
    // data オブジェクトにフォームの入力値が入っています。
  ...
  return (
    <div
      ...
      onClick={onClose} // 背景クリックで閉じる (送信中は無効化も検討)
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ...
        onClick={(e) => e.stopPropagation()} // モーダル内部のクリック伝播を防ぐ
      >
        <div className="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <button
            ...
            aria-label="閉じる"
            disabled={isSubmitting} // 送信中は閉じるボタンも無効化
          >
        ...
        {submitStatus === 'success' ? (
          ...
          // react-hook-form の handleSubmit でラップ
          <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-5">
            ...
            <div>
              ...
              <input
                type="text"
                id="company"
                {...register('company')} // 任意項目なのでバリデーションなし
                className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5"
            ...
            <div className="pt-3 text-right">
              <button
                ...
                disabled={isSubmitting} // react-hook-form の isSubmitting を使用
              >
